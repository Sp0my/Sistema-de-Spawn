...
public class ScreenTooltipManager : MonoBehaviour
{
    ...
    public void SetTooltipAtPosWithMessage(Vector3 pos, string message, bool isTwoDimension = true)
    {
    ...
    private Vector3 GetPos_2D(Vector3 pos) => pos;
    private Vector3 GetPos_3D(Vector3 pos)
    {
        Vector3 screenPos = Camera.main.WorldToScreenPoint(pos);
        return screenPos + PosTorwardCenter(screenPos); 
    }

    private Vector3 PosTorwardCenter(Vector3 pos) => (centerScreenPos - pos).normalized * transformTooltip.rect.width;
} 
